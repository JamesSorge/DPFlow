...
DIR_BACKWARD_FLOW="datasets/kubric_nk/backward_flow/1k"
...
do
    ...
    if [ "$i" -lt "$THRESHOLD" ]; then
        ...
        # # mv content from directories in Backward Flow
        mv ./${DIR_BACKWARD_FLOW}/${DIR_ITERABLE_00}${i}/*.png "${DIR_KUBRIC_1K}/${DIR_ITERABLE_00}${i}"
    else
        ...
        # # mv content from directories in Backward Flow
        mv ./${DIR_BACKWARD_FLOW}/${DIR_ITERABLE_0}${i}/*.png "${DIR_KUBRIC_1K}/${DIR_ITERABLE_0}${i}"
    fi
done